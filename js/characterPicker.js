			...
			function loadCharactersMap(respath,filename) {
				...
				http_request.open("GET", respath + filename, true);
				http_request.onreadystatechange = function () {
					var done = 4, ok = 200;
					if (http_request.readyState == done && http_request.status == ok) {
						list = JSON.parse(http_request.responseText);
            ...
            function loadSelectedBlock(node) {
                ...
                if (node.options[index].value != "") {
                    removeButtons();
                    loadCharactersMap(respath,node.options[index].value);
                }
            ...
            function loadCharactersFile(respath,filename) { // The other way around
                removeButtons();
                loadCharactersMap(respath,filename);
            }
            
			function start(respath) { 
				updateInputLength();
				/* loadFileList('res/list.json'); */
				var listfile = respath + "list.json"
				loadFileList(respath + 'list.json');
				...
				if (queryString['load'] != null) {
				    ...
				    removeButtons();
				    loadCharactersMap(respath,filename.slice(filename.lastIndexOf('/')+1));
				}
